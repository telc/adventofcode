...
input = """bot 49 gives low to bot 118 and high to bot 182
...
class Bot:
  ...
  def __repr__(self):
    return '<Low: ' + ' '.join(self.low) + ' - High: ' + ' '.join(self.high) + ' - Value: ' + ','.join(map(str,self.values)) + '>'
...
for line in sorted(input.split('\n')):
  l = line.split()
  if l[0] == 'bot':
    bots[l[1]] = Bot(l[5:7], l[10:12])
    if l[5] == 'output':
      outputs[l[6]] = None
    elif l[10] == 'output':
      outputs[l[11]] = None
  elif l[0] == 'value':
    bots[l[5]].giveValue(l[1])
...
while running():
  for b in bots:
    ...
    if len(bot.values) > 1:
      if bot.low[0] == 'bot':
        bots[bot.low[1]].giveValue(bot.values[0])
      else:
        outputs[bot.low[1]] = bot.values[0]
      if bot.high[0] == 'bot':
        bots[bot.high[1]].giveValue(bot.values[1])
      else:
