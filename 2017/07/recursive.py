...
def parseList(program_list):
  programs = {}
  for line in program_list.splitlines():
    match = re.match('(\w+) \((\d+)\)(.*)', line)
    children = []
    if match.group(3):
      children = match.group(3)[4:].split(', ')
    programs[match.group(1)] = {"name": match.group(1), "weight": int(match.group(2)), "children": children}
  return programs

def part1(programs):
  parents = {}
  for prog in programs:
    if programs[prog]["children"]:
      parents[prog] = set(programs[prog]["children"])
...
def towerWeight(programs, progs):
  w = 0
  for prog in progs:
    w += programs[prog]["weight"]
    if programs[prog]["children"]:
      w += towerWeight(programs, programs[prog]["children"])
  return w

def part2(programs, base, goal=None):
  weights = {}
  for tower in programs[base]["children"]:
    weights[tower] = towerWeight(programs, [tower])
  values = weights.values()
  value = None
  target = None
  if values.count(max(values)) > values.count(min(values)):
    value = min(values)
    target = max(values)
  elif values.count(max(values)) < values.count(min(values)):
    value = max(values)
    target = min(values)
  else:
    return goal - sum(values)

  for k,v in weights.iteritems():
    if v == value:
      return part2(programs, k, target)

programs = parseList(programs)
base = part1(programs)
print "part1:", base
print "part2:", part2(programs, base)
